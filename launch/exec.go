...
func Start(ctx context.Context, cfg *config.Config) error {
	...
	brokAPIs := []shipx.RouteRegister{ // tunnel API，用于 manager-broker 内部业务通信。
		brkrestapi.NewHeartbeat(brkHeartbeatSvc),
	}
	httpAPIs := []shipx.RouteRegister{ // HTTPS 端口兼容 HTTP 请求，仅用于部署逻辑的暴露。
	...
	}
	{
		...
		if err = shipx.RegisterRoutes(rbg, routes); err != nil {
	...
	}
	{
		...
		if err = shipx.RegisterRoutes(rbg, httpAPIs); err != nil {
	...
	}
	{
		...
		if err = shipx.RegisterRoutes(rbg, brokAPIs); err != nil {
