...
func NewEmail(qry *query.Query, pusher push.Pusher) *Email {
	return &Email{
		qry:    qry,
		pusher: pusher,
	}
...
func (biz *Email) Page(ctx context.Context, page param.Pager) (int64, []*model.Email) {
	tbl := biz.qry.Email
...
func (biz *Email) Create(ctx context.Context, req *mrequest.EmailCreate) error {
	dat := &model.Email{
		Host:     req.Host,
...
func (biz *Email) Update(ctx context.Context, req *mrequest.EmailUpdate) error {
	// 查询数据
	id := req.ID
	tbl := biz.qry.Email
...
func (biz *Email) Delete(ctx context.Context, id int64) error {
	// 查询数据
	tbl := biz.qry.Email
	dat, err := tbl.WithContext(ctx).
		Where(tbl.ID.Eq(id)).
